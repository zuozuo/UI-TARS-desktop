...
model
Indicates a newly trained model.

framework
If the paper proposes a new agent framework.

dataset
If a new (training) dataset is created and published.

benchmark
If a new benchmark is established (also add "dataset" if there's a new training set).

primary studies
List the main focus or innovation in the study.

Abbreviations
Include commonly used abbreviations associated with the paper (model names, framework names, etc.).
